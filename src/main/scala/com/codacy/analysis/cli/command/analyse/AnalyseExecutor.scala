...
import com.codacy.analysis.cli.utils.{EitherOps, SetOps}
...
class AnalyseExecutor(toolInput: Option[String],
  ...
  def run(): Future[Either[String, Unit]] = {
    ...
    val analysisResult: Seq[ExecutorResult] = filesTargetAndTool.fold(str => Future.successful(Left(str)), {
      case (filesTarget, tools) =>
        SetOps.mapInParallel(tools, nrParallelTools) { tool =>
          ...
          val uploadResult = uploadResults(tool, analysisResults)
          ExecutorResult(tool.name, EitherOps.fromTry(analysisResults), uploadResult)
        }
    })
  ...
  private def uploadResults(tool: Tool, result: Try[Set[Result]]): Future[Either[String, Unit]] = {
...
object AnalyseExecutor {
  ...
  case class ExecutorResult(toolName: String,
                            analysisResults: Either[String, Set[Result]],
                            uploadResult: Future[Either[String, Unit]])
